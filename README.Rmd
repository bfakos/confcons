...
Three small functions, `thresholds()`, `confidence()` and `consistence()`, belong to the core of the package. A wrapper function called `measures()` utilize these workhorse functions and calculates every measures for you along with some traditional measures, such as AUC and maxTSS.
...
This GitHub version of the package is now in stable state and a manuscript about the novel measures, *confidence* and *consistence*, is submitted to a scientific journal. Upon acceptance we aims to publish 'confcons' to [CRAN](https://cran.r-project.org/).
